...
/************************新添加**************************/
#ifdef	DUOMEITI
void esp_hidd_send_duomeiti_value(uint16_t conn_id, uint16_t key_cmd)
{

    ESP_LOGD(HID_LE_PRF_TAG, "key_cmd = %d", key_cmd);
	//printf("key_cmd = %d\r\n", key_cmd);
    hid_dev_send_report(hidd_le_env.gatt_if, conn_id,
                        HID_RPT_ID_DUOMEITI_IN, HID_REPORT_TYPE_INPUT, sizeof(key_cmd), &key_cmd);
    return;

}
// //发送电池电量通知
// void esp_hidd_send_battery_value(uint16_t conn_id, uint8_t battery)
// {
// 	printf("battery = %d\r\n",battery);
// 	esp_ble_gatts_send_indicate(hidd_le_env.gatt_if,conn_id,incl_svc.start_hdl + BAS_IDX_BATT_LVL_VAL,sizeof(battery), &battery ,false);
